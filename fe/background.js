...
(function () {
  ...
  // ---- Fallback: no WebGL -> keep the CSS poster, expose an inert API. ----
  if (!gl) {
    window.ScentBG = {
      setFamily: function () {},
      setPaused: function () {},
  ...
  // ---- state ----
  var dpr = Math.min(window.devicePixelRatio || 1, window.innerWidth < 768 ? 1.5 : 1.75);
  ...
  window.addEventListener("resize", function () {
    dpr = Math.min(window.devicePixelRatio || 1, window.innerWidth < 768 ? 1.5 : 1.75);
    resize();
    ensureRunning();
  });
